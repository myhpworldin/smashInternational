...
import { addSupportMessageForAdmin } from "@/server/services/supportTickets.service";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await addSupportMessageForAdmin(new ObjectId(id), parsed.data.text, admin);
    if (!result.ok) {
      return NextResponse.json({ ok: false, errors: result.errors }, { status: 400 });
    }
    return NextResponse.json({ ok: true, ticket: result.data }, { status: 200 });
  } catch (error) {
    console.error("[admin/support-tickets/messages] failed:", error);
